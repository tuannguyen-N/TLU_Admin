...
export function EditNotificationCard({ notification, onCancel, onSave }: Props) {
  ...
  useEffect(() => {
    if (availableTargets.length > 0 && form.targetIds.length > 0) {
      const selected = availableTargets.filter(t =>
        form.targetIds.includes(t.id)
      );
      setSelectedTargetObjects(selected);
    }
  }, [availableTargets, form.targetIds]);
          ...
          <Grid>
            <Grid.Col span={12}>
              <Select
                ...
                placeholder="-- Chọn mẫu thông báo (không bắt buộc) --"
