...
import { createNotificationTemplate } from '../services';
import type { NotificationTemplateFormData } from '../types';
...
export function AddTemplateCard({ onCancel, onSave }: Props) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      mantineNotifications.show({ title: 'Thành công', message: 'Tạo mẫu thông báo thành công', color: 'green' });
      onSave();
    } catch (err) {
      setApiError(err instanceof Error ? err.message : 'Đã xảy ra lỗi');
      mantineNotifications.show({ title: 'Lỗi', message: 'Tạo mẫu thông báo thất bại', color: 'red' });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className={classes.page} style={{ position: 'relative' }}>
      ...
      <div className={classes.pageHeader}>
        <h1 className={classes.pageTitle}>Tạo Mẫu Thông Báo Mới</h1>
        <p className={classes.pageSubtitle}>Nhập thông tin chi tiết để tạo mẫu thông báo mới.</p>
      </div>
