...
import { updateApplicationType } from '../services';
...
export function EditApplicationTypeCard({ applicationType, onCancel, onSave }: Props) {
  ...
  const handleSave = async () => {
    ...
    try {
      await updateApplicationType(applicationType.id, {
        code: form.code.trim(),
        name: form.name.trim(),
      });
      notifications.show({
        title: 'Thành công',
        message: 'Cập nhật loại đơn từ thành công',
        color: 'green',
      });
      onSave();
    } catch (err) {
      ...
      notifications.show({
        title: 'Lỗi',
        message: 'Cập nhật loại đơn từ thất bại',
        color: 'red',
      });
    } finally {
      setLoading(false);
