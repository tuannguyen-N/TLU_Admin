...
import { createStudentClass } from '../services';
...
interface Props {
  ...
  onCancel: () => void;
  onSave: (data: StudentClassFormData) => void;
}

export interface StudentClassFormDataInput {
  classCode: string;
...
export function AddStudentClassCard({ selectedFaculty, onCancel, onSave }: Props) {
  const [form, setForm] = useState<StudentClassFormDataInput>({
    classCode: '',
    startYear: null,
    majorId: null,
  });
  ...
  const set = (key: keyof StudentClassFormDataInput) => (val: any) => {
  ...
  const handleSave = async () => {
    ...
    const payload: StudentClassFormData = {
    ...
    try {
      ...
      notifications.show({
        title: 'Thành công',
        message: 'Tạo lớp sinh viên thành công',
        color: 'green',
      });
      onSave(payload);
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Đã xảy ra lỗi khi tạo lớp sinh viên';
  ...
  return (
    <div className={classes.page} style={{ position: 'relative' }}>
      ...
      <div className={classes.pageHeader}>
        <h1 className={classes.pageTitle}>Thêm Lớp Sinh Viên Mới</h1>
        <p className={classes.pageSubtitle}>Nhập thông tin chi tiết để khởi tạo lớp sinh viên trong hệ thống.</p>
      </div>
